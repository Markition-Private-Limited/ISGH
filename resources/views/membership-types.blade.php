...
<head>
  ...
  <style>
    ...
    */
    ...
    .zip-msg, .phone-msg, .email-msg { font-size: 12px; margin-top: 4px; min-height: 16px; }
    .zip-msg.error,   .phone-msg.error,   .email-msg.error   { color: #dc2626; }
    .zip-msg.success, .phone-msg.success, .email-msg.success { color: #10b981; }
  ...
  <script>
    ...
    async function checkMemberEmail(input) {
      ...
      if (msgEl) { msgEl.textContent = ''; msgEl.className = 'email-msg'; }

      if (!email) return;

      // Show "checking…" while waiting for a valid format
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        if (msgEl) { msgEl.textContent = 'Enter a valid email address.'; msgEl.className = 'email-msg error'; }
        return;
      }

      if (msgEl) { msgEl.textContent = 'Checking…'; msgEl.className = 'email-msg'; }
      ...
      try {
        ...
        if (data.exists) {
          ...
          if (msgEl) { msgEl.textContent = 'This email is already taken — already registered as an ISGH member.'; msgEl.className = 'email-msg error'; }
        } else {
          input._waEmailExists = false;
          input.style.borderColor = '#10b981';
          if (msgEl) { msgEl.textContent = '✓ Email is available'; msgEl.className = 'email-msg success'; }
        }
      } catch (e) {
        input._waEmailExists = false;
        if (msgEl) { msgEl.textContent = ''; msgEl.className = 'email-msg'; }
    ...
    async function submitMembership(type) {
      ...
      // Block if any non-primary member email is already taken in WA
      if (sectionEl) {
        const memberEmails = [...sectionEl.querySelectorAll('input[type="email"]:not([readonly])')];
        for (const emailInput of memberEmails) {
          ...
          // Also block if field has a value but check hasn't resolved yet
          const val = emailInput.value.trim();
          if (val && emailInput._waEmailExists === undefined) {
            emailInput.focus();
            emailInput.scrollIntoView({ behavior: 'smooth', block: 'center' });
            return;
          }
          ...
          <div class="membership-banner banner-family">
            <div class="banner-left">
              ...
              <div class="banner-badges">
                ...
                <span class="banner-badge">✔ Votting Rights</span>
                <span class="banner-badge">✔ Each Member has seprate login</span>
