    ...
    */
    ...
    <script>
        ...
        function toggleMembershipForm() {
            ...
            // Type changed → reconcile the card-element's pk. If the new type
            // is non-checkomatic this drops back to the env key.
            _syncStripeForCheckomatic();
        }
        ...
        function initStripeCardElement() {
            ...
            _currentPk = _stripePublishableKey;
        ...
        // For checkomatic the card element must tokenize against the zone's
        // Stripe account (not the env default). When type or center changes,
        // ask the server which pk to use, and if it differs from the current
        // one tear down + re-mount the card element under a fresh Stripe()
        // instance. Non-checkomatic types reset to the env pk.
        let _currentPk = '';
        async function _syncStripeForCheckomatic() {
            const type = document.getElementById('membershipSelector')?.value ?? '';
            const isChk = (type === 'checkomatic_family' || type === 'checkomatic_individual');
            const zone = isChk ? (_zipState?.uni?.zone ?? '') : '';

            // Non-checkomatic OR no zone yet → use env default. Skip the fetch.
            const targetType = isChk ? type : '';
            const targetZone = isChk ? zone : '';

            try {
                const res = await fetch('/membership/stripe-key', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content,
                    ...
                    body: JSON.stringify({ zone: targetZone, type: targetType }),
                });
                const data = await res.json();
                const newPk = (data && data.publishable_key) ? data.publishable_key : _stripePublishableKey;
                if (newPk && newPk !== _currentPk) {
                    _reinitStripeJs(newPk);
                }
            } catch (e) {
                console.warn('[Stripe] pk lookup failed, keeping current key', e);
            }
        }

        function _reinitStripeJs(pk) {
            if (!pk) return;
        ...
        async function validateZip(input, prefix) {
            ...
            try {
                ...
                if (centers.length === 1) {
                    ...
                    if (prefix === 'uni' && _isChk) _syncStripeForCheckomatic();
                } else {
                        ...
                        <div class="fields-stack">
                            ...
                            <div class="field">
                                ...
                                <input type="text" id="uni_zip" placeholder="77001"
                                    oninput="onPrimaryAddressChange(); clearTimeout(window._zipDebounce); window._zipDebounce = setTimeout(() => validateZip(this,'uni'), 400)">
                ...
                <div>
                    ...
                    <ul class="flex flex-col gap-2.5" style="font-family:'SF Pro regular';">
                        <li><a href="{{ route('join') }}"
                            class="flex items-center gap-2 text-green-200 text-sm hover:text-white transition-colors"><span class="text-green-400">•</span> Become a Member</a></li>
                        <li><a href="{{ route('membership-verification') }}"
                            class="flex items-center gap-2 text-green-200 text-sm hover:text-white transition-colors"><span class="text-green-400">•</span> Verify Membership Status</a></li>
                        <li><a href="{{ route('member-portal.login') }}"
                            class="flex items-center gap-2 text-green-200 text-sm hover:text-white transition-colors"><span class="text-green-400">•</span> Sign In</a></li>
                        <li><a href="{{ route('join') }}"
                            class="flex items-center gap-2 text-green-200 text-sm hover:text-white transition-colors"><span class="text-green-400">•</span> Join Now</a></li>
                    </ul>
                </div>
                <div>
