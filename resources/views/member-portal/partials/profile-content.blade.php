{{-- Profile page content — cards as direct children of #profilePage.
     They pair up row-by-row via CSS grid auto-placement so each Profile card
     sits beside (and matches the height of) its edit form:
       Row 1 · Membership Information     | Quick Links
       Row 2 · Primary Member form        | Primary Profile card
       Row 3+ · Family Member form        | Family Member Profile card (one pair per associated family member) --}}

{{-- ── Row 1 · left: Membership Information ── --}}
<div class="info-card">
  <div class="info-head">
...
{{-- ── Row 2 · left: Primary Member Information ── --}}
<div class="card">
  ...
  <form class="form-grid" id="formPrimary" novalidate>
    ...
    <div class="field" style="grid-column: 1 / -1;"><label for="p-street">Street Address<span class="req">*</span></label><input type="text" id="p-street" value="{{ $profile->street }}" data-editable readonly /></div>
    ...
    <div class="field"><label for="p-dob">Date of Birth<span class="req">*</span></label><input type="text" id="p-dob" value="{{ $profile->dobMdy() }}" placeholder="MM/DD/YYYY" readonly /></div>
    ...
    <p class="form-note">
      ...
      Only the Street Address field can be edited here. To change any other detail, please contact ISGH.
    </p>
  </form>
</div>

{{-- ── Row 2 · right: Primary Profile card ── --}}
...
{{-- ── Row 3+ · left: Family Member Information ── --}}
<div class="card">
  <div class="card-header">
  ...
  <form class="form-grid" id="formFamily{{ $famIndex }}" novalidate>
    ...
    <div class="field"><label for="f{{ $famIndex }}-dob">Date of Birth<span class="req">*</span></label><input type="text" id="f{{ $famIndex }}-dob" value="{{ $famMember->dobMdy() }}" placeholder="MM/DD/YYYY" readonly /></div>
    <div class="field"><label for="f{{ $famIndex }}-zip">Zip Code<span class="req">*</span></label><input type="text" id="f{{ $famIndex }}-zip" value="{{ $famMember->zip }}" readonly /></div>
    <div class="field"><label for="f{{ $famIndex }}-city">City<span class="req">*</span></label><input type="text" id="f{{ $famIndex }}-city" value="{{ $famMember->city }}" readonly /></div>
    <div class="field"><label for="f{{ $famIndex }}-state">States<span class="req">*</span></label><input type="text" id="f{{ $famIndex }}-state" value="{{ $famMember->state }}" readonly /></div>
...
{{-- ── Row 3+ · right: Family Member Profile card ── --}}
<div class="profile-card spouse-profile">
  ...
  <div class="profile-sub">{{ $profile->level ?: 'Member' }}</div>
  <button type="button" class="mobile-edit-link" onclick="document.getElementById('btnEditFamily{{ $famIndex }}')?.click(); document.getElementById('formFamily{{ $famIndex }}')?.scrollIntoView({behavior:'smooth', block:'start'});">
    <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4z"/></svg>
    Edit Profile
  </button>
</div>
